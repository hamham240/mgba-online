...
Server::Server(const std::uint16_t a_port, std::uint8_t* a_general_buffer, std::atomic_bool& a_stop_token) 
	...
	m_acceptor = std::make_shared<tcp::acceptor>(*m_io_ctx, *m_endpoint);
}

void Server::exec() {
	m_error_code = m_acceptor->accept(*m_socket, m_error_code);
